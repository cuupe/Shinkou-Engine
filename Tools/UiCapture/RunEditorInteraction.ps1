...
$script = (Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot 'EditorInteraction.txt')).Replace('@PROJECT@', $project.Replace('\','/')).Replace("`r`n", "`n")
$scriptPath = Join-Path $output 'interaction.txt'
[IO.File]::WriteAllText($scriptPath,$script, [Text.UTF8Encoding]::new($false))
& (Join-Path $build 'shinkou_ui_capture.exe') (Join-Path $build 'shinkou_engine_sample.exe') (Join-Path $output 'interactive.bmp') 5000 --client-size 1920x1080 --require-gpu --script $scriptPath -- --editor dx11 --asset-view tree --project $project --theme $Theme *> (Join-Path $output 'run.log')
$result = $LASTEXITCODE
