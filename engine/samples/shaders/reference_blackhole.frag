...
layout(set = 1, binding = 2) uniform FrameState {
    vec4 timeFrame;
    vec4 camera;
} frameState;
...
float state_time() {
    return frameState.timeFrame.x;
}

int state_frame() {
    return int(frameState.timeFrame.y + 0.5);
}

#define iTime state_time()
#define iFrame state_frame()
...
#define iPrepass                0      // 禁用预渲染插值，直接按目标分辨率积分 / Direct target-resolution integration
...
#define iEnableHeatHaze         0      // 关闭高成本热浪折射，保持实时帧率 / Disable the expensive heat-haze pass for real-time frame rate
#define iEnableShadowCulling    1      // 开启阴影剔除优化 / Enable Shadow Culling Optimization
...
#define iQuality                0.18   // 原生分辨率实时档位，降低测地线积分步数 / Native-resolution real-time tier
...
void mainImage( out vec4 FragColor, in vec2 FragCoord )
{
    ...
    // 读取 Buffer B 相机数据
    vec4 cameraState = frameState.camera;
    float cameraYaw = (cameraState.r - 0.5) * 6.28318530718;
    float cameraPitch = (cameraState.g - 0.5) * 2.4;
    float cameraDistance = mix(18.0, 26.0, cameraState.b);
    vec3 CamPosWorld = vec3(sin(cameraYaw) * cos(cameraPitch), sin(cameraPitch), cos(cameraYaw) * cos(cameraPitch)) * cameraDistance;
    vec3 fwd = normalize(-CamPosWorld);
    vec3 CamRightWorld = normalize(cross(fwd, vec3(-0.5, 1.0, 0.0)));
    ...
    vec2 Jitter = vec2(RandomStep(Uv, fract(iTime * 1.0 + 0.5)), RandomStep(Uv, fract(iTime * 1.0))) / iResolution.xy;

    TraceResult res = TraceRay(Uv + 0.5 * Jitter, iResolution.xy,
